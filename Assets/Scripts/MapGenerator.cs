...
public class MapGenerator : MonoBehaviour {
    ...
    public CustomGrid<Encounter> GeneratePath(List<int>[] pathList) {
        CustomGrid<Encounter> mapGrid = new CustomGrid<Encounter>(Width, Height, _originPosition.position, _offsetX, _offsetY);
    ...
    private void MakeVertex(CustomGrid<Encounter> mapGrid, Rowcol from, Rowcol to) {
        Encounter prev = mapGrid.GetElement(from.row, from.column);
        Encounter cur = mapGrid.GetElement(to.row, to.column);
        ...
        foreach (Rowcol nextRowcol in cur.AdjustSet) { 
            MakeVertex(mapGrid, to, nextRowcol);
        }
    }
